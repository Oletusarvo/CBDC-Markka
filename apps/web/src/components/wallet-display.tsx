  ...
  return (
    <div className='flex flex-col gap-2 w-full overflow-x-scroll max-h-full h-full flex-1 px-4'>
      {isPending ? (
        ...
        tokens.map((t, i) => <Token data={t} key={`transaction-${i}`} />)
      ) : (
        <div className='flex flex-col justify-center items-center flex-1'>
          <h2 className='text-sm text-slate-500'>Your wallet is empty.</h2>
        </div>
      )}
    </div>
  );
...
export function TokenStatusStamp({
  ...
  return (
    <div className={containerClassName}>
      {status === "spendable" ? (
        <StampText>Active</StampText>
      ) : status === "limited" ? (
        <StampText>Restricted</StampText>
      ) : (
        <StampText>Expired</StampText>
      )}
    </div>
  );
}
